...
contract EggAuctionWar is EggAuction{
    ...
    event WarExecuted(
        ...
        uint256 time,
        uint8 eType);
    function joinWar(uint256 _eggTokenId,uint8 _eType) public whenNotPaused{
        require(_eggTokenId >= 0);
        require(_eType >= 0 && _eType <= 20);
        require(_getEggCount(msg.sender,_eType)>0);
        bool flag = false;
        uint8 i;
        for(i=0; i< mappEggMk[msg.sender].length; i++){
            if(mappEggMk[msg.sender][i] == _eggTokenId){
                flag = true;
                break;
            }
        }
        require(flag);
        require(
            _transferEggMk(msg.sender,owner,1,_eType,0)
        );
        eggWar[msg.sender] =  new EggWar(msg.sender,1,now,_eType,0);
        WarCreated(msg.sender,_eggTokenId,now,_eType);
    }
    function finishWar(address _from,uint256 _count,uint8 _eType,uint256 _cardTokenId) public whenNotPaused onlyOwner {
        require(eggWar[_from] != null && eggWar[_from].count>0);
        WarExecuted(_from,_cardTokenId,_cardTokenId,_count,now,_eType);
        delete eggWar[_from];
    }
}
